...
PROC TRANSPOSE DATA=H
	...
	ID Indicateur;
RUN;	
/* on group par med obs et chirurgie*/
proc sql;
	create table H61 as 
...
proc sql;
	create table score17 as 
		select nom, cat, nbAccouchement/nbLitObstretique as score
		from exo8a 
		where nbAccouchement/nbLitObstretique > 0
		order by score DESC;
run;
/*ON REMARQUE QUE LES ETABLISSEMENTS IMPORTANT DANS LE DOMAINE DE 
L'OBSTRETIQUE SONT CLASSES DE HAUT EN BAS: CH ST JOSEPH ST LUC, CH GIVORS, 	HÔPITAL NECKER ENFANTS MALADES,...*/
...
run;
/*ON REMARQUE QUE LES ETABLISSEMENTS IMPORTANT DANS LE DOMAINE DE 
L'OBSTRETIQUE SONT CLASSES DE HAUT EN BAS: CH ST JOSEPH ST LUC, CH GIVORS, CLINIQUE AMBROISE PARE,...*/	
...
run;
/*ON REMARQUE QUE LES ETABLISSEMENTS IMPORTANT DANS LE DOMAINE DE 
L'OBSTRETIQUE SONT CLASSES DE HAUT EN BAS: CH ST JOSEPH ST LUC, CH DU BELVEDERE, CLINIQUE SAINT-CYPRIEN RIVE GAUCHE,...*/	
...
proc sql;
	...
	select s1.nom, s1.cat, (s2.score-s1.score) + (s3.score-s2.score) as diff_score
	from score17 as s1 
...
/*il suffit de grouper nos score par categorie */
/*12 */
/*2017*/
proc sql;
